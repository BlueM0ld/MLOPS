...
class DocDataset(Dataset):
    def __init__(self, df_input):
        self.queries = df_input['query']
        self.docs_rel = df_input['doc_rel_tokens']
        self.url = df_input['url_relevant']
...
class TwoTowerModel(nn.Module):
    ...
    def doc_encode(self, doc_ids, doc_mask=None):
        ...
        doc_embed = doc_embed * \
            doc_mask.unsqueeze(-1) if doc_mask is not None else doc_embed
    ...
    def query_encode(self, query_ids, query_mask=None):
        ...
        query_embed = query_embed * \
            query_mask.unsqueeze(-1) if query_mask is not None else query_embed
...
__all__ = ['DocumentDataset', 'TwoTowerModel', 'DocDataset']
