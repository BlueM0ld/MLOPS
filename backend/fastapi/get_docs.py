...
def setup_model():
    ...
    return model, index, df, word_to_idx
...
def search_docs(query,  model, index, df, word_to_idx):

    documents, urls, distances = get_nearest_neighbors(
        query, model, df, 5, index, word_to_idx)
...
#     documents, urls, distances = get_nearest_neighbors(
#         q, model, df, 5, index, word_to_idx)
#     doc = documents.to_list()
#     url = urls.to_list()
#     rel_docs_sim = distances.tolist()
#     return doc, rel_docs_sim
