...
def tokenize(df, word_to_idx):
    ...
    df[['doc_rel_tokens', 'doc_irr_tokens', 'query_tokens']] = df.apply(
        tokenize_row, axis=1, result_type='expand')
    return df


def tokenize_2(df, word_to_idx):
    # Tokenize all columns in one go
    def tokenize_row(row):
        return (
            str_to_tokens(row['doc_relevant'], word_to_idx),
...
__all__ = ['preprocess_list', 'str_to_tokens',
           'preprocess_query', 'simple_preprocess', 'str_to_list']
